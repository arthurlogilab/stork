...
import (
	...
	vol_driver "github.com/portworx/torpedo/drivers/volume"
...
func (s *SSH) initExecPod() error {
	...
	if ds, err = k8sops.GetDaemonSet(execPodDaemonSetLabel, execPodDefaultNamespace); ds == nil {
		...
		specFactory, err := spec.NewFactory(fmt.Sprintf("%s/%s", defaultSpecsRoot, execPodDaemonSetLabel), vol_driver.GetStorageProvisioner(), s)
		if err != nil {
